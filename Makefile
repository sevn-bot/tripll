...
MERGECRAFT_REF ?= $(if $(TRIPLL_MERGECRAFT_REF),$(TRIPLL_MERGECRAFT_REF),b8e83a82e97ed537706d9a712e59af9ef031588f)
...
.PHONY: help sync sync-api init tripll lint typecheck test check ci-steps ci-resume ci-reset ci-affected ci-changed partial-ci deps-audit log-redact-check mergecraft-ref-check module-size-check rules-check review serve status-watch orchestrator-watch \
...
check: lint typecheck log-redact-check mergecraft-ref-check about-site-check test ## Lint + typecheck + log redact + mergeCraft pin + about-site drift + test (required gate)
...
module-size-check: sync ## Fail when any non-allowlisted src/tripll module exceeds 1000 lines
	$(UV_RUN) run --extra dev python scripts/check_module_size.py
...
CI_STEPS := lint typecheck log-redact-check mergecraft-ref-check module-size-check about-site-check rules-check test deps-audit build
