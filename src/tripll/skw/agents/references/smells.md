# Fowler smell baseline (reviewer — Standards axis)

**Provenance:** harvested from `mattpocock/skills/engineering/code-review` (MIT); smell set is
Martin Fowler, *Refactoring*, ch. 3. Referenced by [`../reviewer.md`](../reviewer.md) and
[`../../prompts/reviewer.md`](../../prompts/reviewer.md).

Fixed 12-smell checklist the **reviewer** applies to every branch diff, on top of the configured
review plugin (default: thermo) and the [`constitution.md`](../../constitution.md) MUST-check.
Two rules bind the whole list:

- **The repo overrides.** A documented repo standard (`constitution.md`,
  `about-sevn.bot/_standards/coding-standards.md`, or similar) always wins — where it endorses
...
- **Always a judgement call.** Each smell is a labelled heuristic ("possible Feature Envy"), never
  a hard violation — and, like any standard here, skip anything tooling already enforces (ruff,
  mypy, etc.).

Each entry reads *what it is* → *how to fix*; match it against the diff hunk-by-hunk:
...
## How this fits the two-axis review

This baseline is part of the **Standards** axis (plugin + constitution + this list) — kept
separate from the **Spec** axis (does the diff faithfully implement the originating spec,
user stories, or wave-file section). See `agents/reviewer.md` Role step 4–5: never rerank or
merge findings across the two axes.
