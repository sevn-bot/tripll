...
### Changed
- [2026-08-03] God-module extraction (#62): split `inject.py` and `skw/render.py` via ADR 013 façade re-exports into sibling modules; zero caller import edits; module-size allowlist cleared — `make module-size-check` now enforces ≤1000 lines with no exemptions
...
### Added
- [2026-08-03] Review bench runner (#64 W5): `make bench-review` / `tripll bench run --track review -k 3` scores Harbor review tasks against frozen baseline; F1 delta written to `.tripll/bench-review-latest.json`; nightly workflow fails on configured regression threshold
...
- [2026-08-03] Release process documented (#65): Craft evaluated and rejected; tag-triggered GitHub Actions release workflow and operator checklist in `docs/runbooks/release.md`
