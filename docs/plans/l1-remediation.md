...
| **Stage** | Pre-flight — nothing dispatched |
| **Next action** | P0.1 — clear the GitHub Actions billing block on the `sevn-bot` org |
| **Blocked on** | P0.1 (external, human-or-auto-accept — see *Human gates*) |
| **Last pushed sha** | — |
| **Last CI run id** | — (zero successful runs in repo history) |
| **Parked waves** | 0 of 3 (plan-level stop rule) |
| **Integration target** | *undecided — resolved by the P0.1 rule, see W0.2* |
...
|------|------------------|-------|----------|--------|
| P0 | **human** + `cursor_local` claude-opus-5 | Pre-0 gate: clear Actions billing; CI timeout + Python pin; Make `sync` prereqs; plan self-hosting | CI-00, DX-01, DX-02, DX-05, PLAN-selfhost, PLAN-gates, SHAPE-01 | [ ] |
| P1 | `cursor_local` auto | **Provider fabric**: per-provider pools, per-wave routing, infra-failure class, failover, model-ID refresh, **effort + budget flags**, auth preflight, ceiling calibration | PROV-01…03, MODEL-01, EFFORT-01, BUDGET-01, AUTH-01, CAP-01 | [ ] |
...
- [ ] **P0.1** *(human, external — tier-4 canary)* Clear the GitHub Actions billing block on the
      `sevn-bot` org — settings → Billing & plans.
- [ ] **P0.2** Add `timeout-minutes: 20` to the CI job (DX-01). Test wall time is 18.7s; 20 minutes
      is generous and still bounds a hung async test.
- [ ] **P0.3** Pin `python-version: "3.12"` in `.github/actions/bootstrap/action.yml` (DX-02) to
      match `requires-python = ">=3.12"` and ruff's `target-version = "py312"`.
- [ ] **P0.4** Add `sync` as a prerequisite to `lint` and `typecheck` in the `Makefile` (DX-05),
      matching the existing `pullfrog-ref-check: sync` pattern at `Makefile:295`.
- [ ] **P0.5** Verify `make ci` is reachable end-to-end on a **fresh clone** in a temp dir —
      expect it to still fail on TEST-03 (that is W2's job) and **nothing else**.
- [ ] **P0.6** **Commit + push** (`ci: restore executable gate with timeout and python pin`).
- [ ] **P0.7** *(PLAN-selfhost)* Teach `plan_paths` a **planned-new** exemption: paths listed in the
      ...
      file this plan will author is reported as a dead ref. Ships with its own test.
- [ ] **P0.8** *(PLAN-gates)* Add the human-gate config: `[pipeline] human_gates` +
      ...
      Ships with its own test. Document in the operator runbook.
- [ ] **P0.10** *(SHAPE-01)* Fix the stop-rule proxy. `check_stop_rule` (`shape_checks.py:186–199`)
      ...
      **Do not simply raise the constant** — that disables the guard for every plan tripll runs.
      ...
      targeting >5 modules is still refused.
- [ ] **P0.11** **Commit + push** (`feat(plan): self-host the remediation plan and add gate config`).
...
- [ ] **P1.1** *(PROV-02)* Add `src/tripll/adapters/pools.py`: a `dict[str, asyncio.Semaphore]`
      keyed by backend, built from `[providers.*] max_parallel`, plus the existing global
      `Semaphore(max_parallel)`. Acquire **global → provider**, release in reverse — one fixed order,
      no deadlock. `TRIPLL_MAX_PARALLEL` keeps working as the global ceiling.
      Replace `engine.py:960`'s single `self._sem` with the pool registry.
- [ ] **P1.2** *(PROV-01)* Add `provider: str | None`, `agent: str | None`, and
      `fallback: list[str]` to `WaveNode` (`graph.py:116` already has `model`). Parse them from the
      v3 wave table.
- [ ] **P1.3** *(PROV-01)* Resolve the adapter **per node** in `_execute_node`:
      `build_adapter(node.provider or default_provider, BackendOptions(model=…, agent=…))`.
      `self.adapter` becomes the run default, not the only adapter. `ledger.attempts.backend` is
      already per-attempt (`ledger.py:161`) — record the resolved backend there, no schema change.
- [ ] **P1.4** *(PROV-03)* Add `src/tripll/adapters/failure_class.py`: classify a dispatch outcome
      as `success | failure | infra`. `infra` matches *"Couldn't start"*, *"Workspace
      Disconnected"*, and non-zero exit with empty agent output. An `infra` result **does not**
      consume a wave attempt and **does not** trip the exit-7 breaker — it is retried after the
      provider's `cooldown_s`, and appended as its own ledger event so it stays visible.
- [ ] **P1.5** Adaptive throttle: N consecutive `infra` results from one provider halve its pool
      and start `cooldown_s`; one clean dispatch restores a step. This is W6's per-run breaker
      (BUG-07) scoped **per provider** — reuse that machinery, do not fork it.
- [ ] **P1.6** Failover: when a wave's provider is in cooldown or saturated past the wave deadline,
      re-dispatch on the first entry of `fallback`. Record the switch on the attempt. **Provider
      only** — never a silent model downgrade (R16).
- [ ] **P1.7** *(MODEL-01)* Refresh model IDs. `claude-3-5-sonnet` in `engine.py:24` is **retired**
      and 404s; `DEFAULT_MODEL = "claude-sonnet-4-6"` (`claude_code.py:37`) is a generation behind.
      Set `DEFAULT_MODEL = "claude-sonnet-5"`, correct the Engine docstring to match, and add a
      test asserting the two agree so they cannot drift again.
- [ ] **P1.8** *(EFFORT-01)* Wire reasoning effort. Add `reasoning_effort` to `WaveNode` and emit
      `--effort <level>` in `claude_code.build_argv` (`claude_code.py:207–226`), accepting
      `low|medium|high|xhigh|max` and rejecting anything else at parse time rather than at dispatch.
      **Cursor needs no code change** — `cursor_local.py:131` already forwards `brief["model"]`, so a
      wave declaring `claude-opus-5-thinking-high` or `claude-opus-5[effort=high]` works today.
      Document the split: on Claude Code effort is a flag, on Cursor it is part of the model string.
- [ ] **P1.9** *(BUDGET-01)* Emit `--max-budget-usd` from a per-wave `max_budget_usd` when set. This
      is a **process-level backstop**, independent of tripll's ledger — it holds even when cost
      accounting is wrong, which BUG-cost shows it can be. Record the cap on the attempt so an
      abrupt exit is diagnosable rather than mysterious.
- [ ] **P1.10** **Do not wire `claude --fallback-model`.** It substitutes a *model* on overload,
      which is the silent downgrade R16 forbids. Record the rejection in ADR 010 so the next reader
      does not "helpfully" add it.
- [ ] **P1.11** *(AUTH-01)* Add a per-provider **auth preflight** at run start: verify each provider
      the plan routes to can authenticate, and fail fast with a named provider if not. Under
      `human_gates = "auto_accept"` an interactive auth prompt is a silent hang, not a gate —
      classify a mid-run auth failure as `infra`, park the wave, and surface the provider name.
- [ ] **P1.12** *(CAP-01)* **Calibrate the ceiling, do not just accept it.** Run a bounded probe —
      2, 3, 5, 8 concurrent `cursor_local` dispatches across separate worktrees — and record the
      level at which "Couldn't start" / "Workspace Disconnected" first appears. Record it in the
      runbook with the machine it was measured on. `max_parallel = 5` is the operator's starting
      number (R19); this task replaces it with evidence, and settles whether the limit is
      per-agent or per-workspace.
- [ ] **P1.13** Document the fabric in the operator runbook: the per-provider table, why
      `cursor_local` is capped, what an `infra` event means in the dashboard, how effort is
      expressed per provider, and the auth-preflight failure mode.
- [ ] **P1.14** **Commit + push** (`feat(adapters): per-provider pools, routing, effort, budget`).
...
- [ ] **W11.1** Replace the hand-rolled TOML parse in `log_redact.py:54` with stdlib `tomllib`
      (DX-03) — the current line-splitting parser is fragile for a security-relevant config.
- [ ] **W11.2** Keep `log-redact-check` green against the expanded W4 list.
- [ ] **W11.3** Rebaseline the 7 open dependabot PRs **now that CI runs** (R12): `ruff 0.15.18`,
      `mypy 2.1.0`, `pytest 9.1.1`, `typer 0.26.7`, `uvicorn>=0.49.0`, `actions/checkout@7`,
      `softprops/action-gh-release@3`.
- [ ] **W11.4** Treat **ruff 0.15.18 first and carefully** — the pinned `0.15.12` is what makes the
...
- [ ] **W11.5** **Commit + push** (`chore(deps): rebaseline dependencies under a live gate`).
...
- [ ] **W13.1** *(ONB-01)* Add `src/tripll/config.py`: a single `load_config()` resolving four layers,
      ...
      *under* env, it does not replace it.
- [ ] **W13.2** Adopt SKW's **merge-table precedence rules** from `agent_config.py:257–274`
      (`[agent]` → `[agent.models.<id>]` → per-wave → per-stage → env). Take the resolution order,
      which is well designed; leave the argv builder, which `adapters/` already owns.
- [ ] **W13.3** *(ONB-01)* Add **`tripll setup`**: interactive by default, `--non-interactive` for CI.
      ...
      defined. Writes `~/.config/tripll/config.toml`. Re-running edits rather than replaces.
- [ ] **W13.4** *(R24)* **Never store a credential.** `setup` verifies auth by calling
      ...
      `forbidden` clause because it is the most likely thing to be "improved" later.
- [ ] **W13.5** *(ONB-01)* Add **`tripll doctor`**: preflight that reports Python version, installed
...
- [ ] **W13.6** *(ONB-06)* Move the **v3** wave-plan template into the package as
      ...
      as a symlink or a generated copy so there is **one** source of truth, and assert they match.
- [ ] **W13.7** Prune the `force-include` block in `pyproject.toml`. **Proven dead, not assumed:**
...
- [ ] **W13.7a** Add the guard the `force-include` block was pretending to be: a test that builds (or
      ...
      `force-include` list never did.
- [ ] **W13.8** Adapt SKW's **`nextstep.py`** into `src/tripll/onboard/nextstep.py` — "what is the next
      ...
      `tripll doctor --next` and in the `init` epilogue.
- [ ] **W13.9** Reuse SKW's `runtime.py` (`is_dryrun`, `is_pytest`, `is_auto_approve`) rather than
      re-deriving those checks; they are trivial and already correct.
- [ ] **W13.10** **Commit + push** (`feat(config): tripll.toml, one-time setup, and doctor preflight`).
