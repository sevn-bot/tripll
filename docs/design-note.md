...
| **L1 outer wave dispatch** | With `[graph]` installed, `Engine._drive` routes batch dispatch through `l1_outer` → `waves` → `dispatch_bridge.invoke_engine_wave_dispatch` → `Engine.drive_wave_batches` (L2-W4). Ledger remains authoritative; checkpoints derived. | **verify → commit → review → generate** outer nodes remain step markers (future L2). Orchestrator serial mode bypasses the outer graph. |
