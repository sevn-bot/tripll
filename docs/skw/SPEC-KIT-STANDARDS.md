...
Kit root: **`src/tripll/skw/`** (absorbed into tripll; invoke via `uv run skw …` or Makefile
...
| spec-kit command | skw phase | Artifact | Command |
|------------------|-----------|----------|---------|
| `/speckit.constitution` | governing principles | `src/tripll/skw/constitution.md` | edit file directly |
| `/speckit.specify` | `specify` (front end) | `spec/<slug>/spec.md` | `make specify-run SLUG= TITLE= [CONTEXT=] [PATHS=]` |
| `/speckit.clarify` | `clarify` (front end) | `spec/<slug>/spec.md` Clarifications | `uv run skw render --stage clarify --slug … --title …` |
| `/speckit.plan` | `plan` (front end) | `spec/<slug>/plan.md` | `uv run skw render --stage plan --slug … --title …` |
| `/speckit.tasks` | `tasks` = wave-generator | `src/tripll/skw/waves/<slug>-wave-plan.md` (v2) | `make wave-generator-run SLUG= TITLE=` |
| `/speckit.analyze` + `/speckit.checklist` | reviewer + `problem-types.md` | `review-result.json` | `uv run skw render --wave … --stage review` |
| `/speckit.implement` | wave-runner + loop | branch commits | `uv run skw run --wave …` |

Each front-end phase also has a headless `-run` Makefile variant where listed above, or
`uv run skw render --stage …` for clarify/plan. Prompts dispatch through `scripts/agent.sh`.

## What each standard maps to

- **Constitution** ([`constitution.md`](../../src/tripll/skw/constitution.md)) — spec-kit's foundational standard.
...
- **Spec** ([`spec-templates/spec-template.md`](../../src/tripll/skw/spec-templates/spec-template.md)) — prioritized,
...
- **Plan** ([`spec-templates/plan-template.md`](../../src/tripll/skw/spec-templates/plan-template.md)) — tech stack,
  architecture, Constitution Check, and the `make` verify targets waves will run.
- **Tasks** ([`wave-plan-template.md`](../../src/tripll/skw/wave-plan-template.md)) — the wave-file v2. spec-kit task
  ...
  - **tests-first** ordering via the single `role = test-author` wave (RED before GREEN) — the
    ...
    [`agents/test-creator.md`](../../src/tripll/skw/agents/test-creator.md) / [`prompts/test-creator.md`](../../src/tripll/skw/prompts/test-creator.md);
  - optional `[spec]` TOML table recording `constitution`/`spec`/`plan` provenance (ignored by
    the validator).
- **Implement** — the existing LangGraph loop (`uv run skw run`): per-wave run → verify → commit →
...
- [`skills/codebase-design/SKILL.md`](../../src/tripll/skw/skills/codebase-design/SKILL.md) owns **module / interface /
  ...
  smell baseline reasons about (see [`agents/references/smells.md`](../../src/tripll/skw/agents/references/smells.md)).
...
- [`skills/domain-modeling/SKILL.md`](../../src/tripll/skw/skills/domain-modeling/SKILL.md) owns **ubiquitous
...
```bash
# Review/ratify principles — edit src/tripll/skw/constitution.md
make specify-run  SLUG=my-feature TITLE="My feature" CONTEXT=brief.md PATHS=src/foo
uv run skw render --stage clarify --slug my-feature --title "My feature"
uv run skw render --stage plan --slug my-feature --title "My feature"
make wave-generator-run SLUG=my-feature TITLE="My feature"   # → src/tripll/skw/waves/my-feature-wave-plan.md
make validate WAVE=src/tripll/skw/waves/my-feature-wave-plan.md
uv run skw run --wave src/tripll/skw/waves/my-feature-wave-plan.md   # run → review → (generate) → loop
```

You can skip phases for small changes (author the wave-file directly with
`uv run python -m tripll.skw.scaffold <slug> "<title>" --kit-root src/tripll/skw` or
`make wave-generator-run`), but the constitution and the tests-first wave still apply.
...
| Agent | [`agents/prd-author.md`](../../src/tripll/skw/agents/prd-author.md) |
| Template | [`prd-templates/prd-template.md`](../../src/tripll/skw/prd-templates/prd-template.md) |
| Rules | [`prd-templates/prd-rules.toml`](../../src/tripll/skw/prd-templates/prd-rules.toml) |
| Spec EARS patterns | [`spec-templates/acceptance-criteria-ears.md`](../../src/tripll/skw/spec-templates/acceptance-criteria-ears.md) |

```bash
uv run python -m tripll.skw.prd_validate about-sevn.bot/prd/05-cost-and-providers.md --kit-root src/tripll/skw
make prd-check    # all PRDs under PRD_DIR (default docs/prd)
```
...
| Artifact | Path |
|----------|------|
| Setup skill | [`skills/verifier-setup/SKILL.md`](../../src/tripll/skw/skills/verifier-setup/SKILL.md) |
| Verify template | [`skills/verifier-setup/assets/verify.template.md`](../../src/tripll/skw/skills/verifier-setup/assets/verify.template.md) |
| Agent | [`agents/verifier-setup.md`](../../src/tripll/skw/agents/verifier-setup.md) |
...
| Artifact | Path |
|----------|------|
| Triage skill | [`skills/github-issue-triage/SKILL.md`](../../src/tripll/skw/skills/github-issue-triage/SKILL.md) |
| Policy | [`skills/github-issue-triage/references/triage-policy.md`](../../src/tripll/skw/skills/github-issue-triage/references/triage-policy.md) |
| Wave brief template | [`skills/github-issue-triage/assets/issue-wave-brief.template.md`](../../src/tripll/skw/skills/github-issue-triage/assets/issue-wave-brief.template.md) |
| Agent | [`agents/github-issue-triage.md`](../../src/tripll/skw/agents/github-issue-triage.md) |
